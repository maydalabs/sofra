...
import { PostgresSofraOperatorReadRepository } from './repository'
...
describe('PostgresSofraOperatorReadRepository', () => {
  it('maps each privileged read through an explicit operator record', async () => {
    const repository = new PostgresSofraOperatorReadRepository(
      createGateway(),
      actor,
    )
