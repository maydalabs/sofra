...
/** safety_incidents.severity is text constrained by CHECK, not an enum type. */
const incidentSeverities = ['low', 'medium', 'high', 'critical'] as const
type IncidentSeverity = (typeof incidentSeverities)[number]

function toSeverity(value: string): IncidentSeverity {
  if (!(incidentSeverities as readonly string[]).includes(value)) {
    throw new RepositoryDataError('safety_incidents.severity')
  }
