...
import { PostgresSofraPartnerReadRepository } from './repository'
...
describe('PostgresSofraPartnerReadRepository', () => {
  it('maps the actor-scoped read model into safe referral metrics', async () => {
    ...
    const repository = new PostgresSofraPartnerReadRepository(gateway, partner)
  ...
  it('rejects non-partners before querying the gateway', async () => {
    ...
    const repository = new PostgresSofraPartnerReadRepository(
      { readReferralSummary },
